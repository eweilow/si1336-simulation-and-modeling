...
<p>
  When <InlineTex texContent={"$$r < r_\\infty$$"} />, rounding doesn't affect
  the system.
</p>

<p>
  When <InlineTex texContent={"$$r > r_\\infty$$"} />, rounding <b>does</b>{" "}
  affect the system.
</p>
...
<p>
  The sign of <InlineTex texContent={"$$\\lambda$$"} /> is positive for{" "}
  <InlineTex texContent={"$$0.76 < r < r_\\infty$$"} />, that is:{" "}
  <em>if the system is not chaotic the sign is positive.</em>.
</p>
...
<p>
  Can see a clear dependence on <InlineTex texContent={"$$x_0$$"} /> for the
  Lyapunov exponent.
</p>
...
<p>
  Using a small timestep of <InlineTex texContent={"$$\\Delta t = 0.00008$$"} />{" "}

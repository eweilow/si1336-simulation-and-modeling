...
#define DATA_NAME "../../data/1011_a.bin"
...
int main()
{
  ...
  for (unsigned long n = 0; n < steps; n++)
  {
    ...
    relaxations[n] = runRelaxationsUntilAccuracy(NULL, LINEAR_DIMENSION, value, WANTED_ACCURACY_PERCENT / 100.0, &relaxGaussSeidel, &initialValue, &boundaryValue, &guessPotential, NULL);

    printf("%f %d: %d\n", logarithm, value, relaxations[n]);
  }

  FILE *f = fopen("../../data/1011_a_relaxations.bin", "wb");
